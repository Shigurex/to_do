...
abstract public class BaseComponent {
	...
	public void checkLogin(SignUp signup, Start login) {
		...
		else if (SQL.checkUser(_username, password) == true)
		{
			...
			_frame.changePanelMenu(list.create(), menu.create());
		}
		else
			_frame.changePanel(login.create());
	}

	public void checkSignUp(SignUp signup, Start login) {
		_username = _username_field.getText();
	...
	protected class ButtonAction implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (cmd.equals("Register"))
				...
				_frame.changePanelMenu(myinfo.create(), menu.create());
			else if (cmd.equals("Go to Edit"))
				_frame.changePanelMenu(myinfo_edit.create(), menu.create());
			else if (cmd.equals("Complete Editing"))
				_frame.changePanelMenu(myinfo_edit.create(), menu.create());
			else if (cmd.equals("My List"))
				_frame.changePanelMenu(list.create(), menu.create());
			else if (cmd.equals("Users"))
				_frame.changePanelMenu(users.create(), menu.create());
			else if (cmd.equals("Logout"))
				_frame.changePanel(login.create());
			else if (cmd.equals("Exit"))
				System.exit(0);
			
			_frame.changePanel(base_component.create());
		}
	}
